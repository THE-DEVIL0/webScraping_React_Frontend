...
const ShopifyScraper = ({
  ...
  const platformConfig = {
    name: 'Shopify',
    placeholder: 'https://store.myshopify.com/products/PRODUCT_HANDLE or https://store.myshopify.com/collections/COLLECTION',
    color: 'from-green-500 to-teal-500',
    icon: '🛒',
    apiEndpoint: 'http://127.0.0.1:8001/api/scrapers/shopify',
    scrapeParams: {
      max_products: 30,
      headless: true
    }
  ...
  const startScraping = async () => {
    ...
    if (url.includes('/products/')) {
      addLog(`ℹ️ Single item URL detected, collecting up to ${maxProducts || platformConfig.scrapeParams.max_products} images`, 'info');
    }
